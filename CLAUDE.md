...
8. **stage8_logistics** — Logistics volume estimation with 4-layer cascade + post-validation:
   ...
   - **Post-validation fixes** (after the 4 layers):
     ...
     - **Fix 5 — supplier 00860 dims from description**: supplier 00860 (MB- decor/furniture) with m3_logístico < 0.01 → extract NxNxN from product description (DIM_REGEX), compute vol, apply ratio_subfamilia. estimation_layer='ratio_desde_descripcion_fix5', confidence 0.45 (~284 productos). Excludes CUADRO/ESPEJO/LAMINA/DECORACION/etc. (legitimately flat/decorative). Moves from capa 4 → capa 3.
   - Sets job.status = 'completed' (stage8 is the completion gate)
   - Coverage on real dataset: 100% (14,323/14,324), total 5,559.75 m³
